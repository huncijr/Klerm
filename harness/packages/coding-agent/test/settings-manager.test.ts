...
describe("SettingsManager", () => {
	...
	describe("preserves externally added settings", () => {
		it("should preserve enabledModels when changing thinking level", async () => {
			...
			// Create SettingsManager (simulates the app starting up)
	...
	describe("project settings directory creation", () => {
		it("should not create the app config folder when only reading project settings", () => {
			// Create agent dir with global settings, but no app config folder in project
			const settingsPath = join(agentDir, "settings.json");
			writeFileSync(settingsPath, JSON.stringify({ theme: "dark" }));

			// Delete the app config folder that beforeEach created
			rmSync(join(projectDir, CONFIG_DIR_NAME), { recursive: true });

			// Create SettingsManager (reads both global and project settings)
			const manager = SettingsManager.create(projectDir, agentDir);

			// The app config folder should not have been created just from reading
		...
		it("should create the app config folder when writing project settings", async () => {
			// Create agent dir with global settings, but no app config folder in project
			const settingsPath = join(agentDir, "settings.json");
			writeFileSync(settingsPath, JSON.stringify({ theme: "dark" }));
			...
			// The app config folder should not exist yet
			expect(existsSync(join(projectDir, CONFIG_DIR_NAME))).toBe(false);
			...
			// Now the app config folder should exist
