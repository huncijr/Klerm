...
| Discover and assign coding harnesses | `get_coding_harness_setup`, `refresh_coding_harness_models`, `set_coding_harness_slots` | Available for a global master switch and a dynamic stable-ID agent registry with per-agent harness/On-Off/model/role/effort/tools/specialties setup, fixed shell-free discovery, availability-filtered choices, credential-safe inferred capability profiles, and backend-derived `auto`/`none`/`disabled` routing; slots contain no Personal Bot or memory-profile assignment, and model discovery errors are returned without credential inspection |
...
| Submit and stop work | `prompt`, `steer`, `follow_up`, `abort` | Available; `prompt.targetAgentId` optionally directs one prompt to a validated runnable external agent or configured Klerm Agent 1/2 without enabling a peer pass |
...
The desktop project workspace lists its assigned sessions and opens a selected
session through the normal resume flow. A grounded project question is written
to the active project session, or the first project session when another
project is active. The selected AI is sent as `prompt.targetAgentId`; unavailable
targets fail instead of silently falling back. The selected reasoning effort is
persisted through the existing per-agent setup contract before the prompt starts.
