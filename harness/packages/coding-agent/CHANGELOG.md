...
- Added a sessionless read-only Browser Agent workspace backed by an isolated pinned browser-use worker, an ephemeral credential-free model gateway, exact-origin approvals, DNS public-address checks, typed desktop RPC, and deterministic `.klerm/browser-events.jsonl` audit records. Browser runs use a fixed `browser-agent` audit identity and require a model, start URL, and task but no coding-harness agent slot.
